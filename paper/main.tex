...
six workarounds, measure two, and map
...
implies. Two are measured below: single runs on a developer laptop against
one OpenAI-compatible model (glm-5.1), reproducible from the artifact
...
for are the same ones the OS community is beginning to build.
...
one model, one agent, eight turns---no seeds, CIs, or hardware sweep. They
show the growth \emph{shape} of two walls, not their magnitude.
